...
exports.saveNewUser = async (req, res, next) => {
  try {
    ...
    if (!savedUser) {
      return res
        .status(400)
        .json(new ResponseModel('User could not be created!'));
    }

    console.log(`New user saved with ${savedUser.id}`);
    res.json(new ResponseModel('User was created!', true, savedUser));
  } catch (error) {
    console.error('An error has been occurred while creating user.', error);
    res.status(400).json(new ResponseModel('User could not be created!'));
  }
};
